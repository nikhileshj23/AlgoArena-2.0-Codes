...
void solve(){
    string a;
    cin>>a;
    vector<vector<ll>> dp(a.size(),vector<ll>(2,-1));
    dp[a.size()-1][1]=a[a.size()-1]-'0';
    function<ll(int,int)>f = [&] (int index,int isTight){
        if(index==a.size()){
            return dp[index-1][1];
        }
        if(dp[index][isTight]!=-1) return dp[index][isTight];
        ll ans=0;
        for(int i=0;i<=(isTight?(a[index]-'0'):9);i++){
            if(index>0&&i!=a[index+1]){
                ans+=f(index+1,isTight&&(a[index]==i));
            }
        }
        return dp[index][isTight]=ans;
    };
    cout<<f(0,1)<<endl;
}
