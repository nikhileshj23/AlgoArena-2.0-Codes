...
void solve(){
    ...
    vi v(n); get(v);
    vi preg(n),postg(n);
    preg[0]=v[0];postg[n-1]=v[n-1];
    for(int i=1;i<n;i++) preg[i]=__gcd(preg[i-1],v[i]);
    for(int i=n-2;i>=0;i--) postg[i]=__gcd(postg[i+1],v[i]);
    int ans=max(preg[n-2],postg[1]);
    for(int i=1;i<n-1;i++) ans=max(ans,__gcd(preg[i-1],postg[i+1]));
    cout<<ans<<endl;
}
